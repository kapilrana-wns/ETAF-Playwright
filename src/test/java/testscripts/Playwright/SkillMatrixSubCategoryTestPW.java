...
public class SkillMatrixSubCategoryTestPW extends PlaywrightTestManager {
    @Test(dataProvider = "ExcelDataUsingMaps", dataProviderClass = ExcelDataProviderCustom.class)
    public void createsSubCategory(Map<String, String> data) {
        Assert.assertTrue(tc.Login(props.getProperty("ApplicationUrl"), data.get("UserName"), data.get("Password")));
        Assert.assertTrue(tc.createSubCategory(data.get("BusinessUnitName"), data.get("SubCategoryName"), data.get("Description")));
        Assert.assertTrue(tc.deleteByName(data.get("SubCategoryName")));
    }
}
